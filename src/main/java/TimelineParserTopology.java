...
public class TimelineParserTopology {
    ...
    public static Topology createTopology() {
        ...
        streamsBuilder.stream(INPUT_TOPIC, Consumed.with(Serdes.String(), new JsonSerde<JsonObject>(JsonObject.class)))
                ...
                }).map((key,value) -> new KeyValue<>(value.get("response").getAsJsonObject().get("id").toString(), value.get("response").getAsJsonObject().get("adm").getAsJsonObject())).peek((k, v) ->
        {
            System.out.println(v);


        }).to(OUTPUT_TOPIC, Produced.with(Serdes.String(), new JsonSerde<JsonObject>(JsonObject.class)));
